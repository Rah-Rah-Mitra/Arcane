...
/// preserving both depth and insertion order.
///
/// Returns `(0-based physical page, title, depth_level)` tuples in tree order.
/// Unlike `extract_chapters_with_depth`, this preserves hierarchy for seed loading.
pub fn extract_chapters_with_depth_and_level(
    doc: &Document,
    max_depth: u32,
    ...
    let outlines_ref = match catalog.get(b"Outlines") {
        ...
        Err(_) => anyhow::bail!("no /Outlines in catalog"),
    };
    ...
    let first_ref = match outlines_dict.get(b"First") {
        Ok(obj) => obj.as_reference()?,
        Err(_) => anyhow::bail!("empty /Outlines"),
    };
    ...
    let mut result: Vec<(u32, String, u32)> = Vec::new();
    walk_entries_vec(doc, first_ref, &oid_to_page, 1, max_depth, &mut result);

    if result.is_empty() {
        anyhow::bail!("no usable entries in /Outlines");
    }
    Ok(result)
}

/// Recursively walk outline entries, accumulating (page, title, depth) tuples.
fn walk_entries_vec(
    doc: &Document,
    first_ref: ObjectId,
    ...
    while let Some(item_ref) = current_ref {
        ...
        if let Ok(title_obj) = item_dict.get(b"Title") {
            ...
            if let Some(page_idx) = resolve_dest_page(doc, item_dict, oid_to_page) {
                result.push((page_idx, title, current_depth));
            }
